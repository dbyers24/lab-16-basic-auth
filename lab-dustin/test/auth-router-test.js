...
describe('testing auth router', () => {
  ...
  describe('testing POST /api/signup', () => {
    ...
    it('should respond with a 400', () => {
      ...
      .send({})
      .then(res => {
        console.log('token we go back ', res);

        expect(res.status).toEqual(400);
        // expect(res.text).toExist();
        // expect(res.text.length > 1).toBeTruthy();
      }).catch(err => {
        console.log('ERROR READ*****', err.status, err.message);
      });
    });
  });
  ...
  describe('testing GET /api/login', () => {
    ...
    it('should respond with a 401', () => {
      ...
      .then(userData => {
        tempUser = userData.user;
        console.log('tempUser', tempUser);
        let encoded = new Buffer(`${tempUser.XXXXXX}:${userData.password}`).toString('base64');
        return superagent.get(`${API_URL}/api/login`)
        .set('Authorization', `Basic ${encoded}`);
      })
      .catch(err => {
        console.log('token we go back ', err.text);
